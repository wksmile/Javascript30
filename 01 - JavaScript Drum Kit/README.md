...
[看在线效果](http://kain.site/demo-js30-01/)
...
- **在 window 上添加键盘事件 `keyup` 事件**
- **对应事件处理程序**
    + **获取keyup键码**
    + **移除样式**
    + **将之前保存键码的变量设为undefined**

## 方法比较
方法一中如果连继续按某一个按键，则动画一直连续，最后不能移除样式，这是因为在过滤事件的代码中`if (event.propertyName !== 'transform') return;`,如果是transform或者box-shadow就会出现长按样式不能恢复的情况；可以改为检测其他的属性，原因是浏览器多过渡动画的优化导致的，在连续点击时不明显的动画就不执行了（反正也看不出来），以提高性能。方法二中是设置键盘弹起事件，只要有键盘按键弹起就移除样式，这样无论如何结束时都能移除样式。
